...
export class SwapiService {
    ...
    async getSwapiFilms(): Promise<SwapiFilm[]> {
        const swapiFilmResponse = await this.externalApiService.fetch<SwapiResponse<SwapiFilm>>('https://swapi.dev/api/films')
        return swapiFilmResponse.results
    }

    async getSwapiFilm(id): Promise<SwapiFilm> {
        return this.externalApiService.fetch<SwapiFilm>(`https://swapi.dev/api/films/${id}`)
    }
